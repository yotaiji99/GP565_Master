 ...
 *      EF                 @
 *     0!                         $Header: //depot/release/Embedded/BaseComps/v2.4.8.0/sw/comps/gphal/inc/gpHal_HW_MM.h#1 $
 *    M'   GreenPeak              $Change: 75614 $
 *   0'         Technologies      $DateTime: 2016/01/14 12:38:05 $
 *  F
 */

#ifndef _HAL_GP_HW_MM_H_
#define _HAL_GP_HW_MM_H_
...
{
    *pData =  *((volatile UInt64Struct_t *) Address);
}
...
{
    *((volatile UInt64Struct_t *) Address) = *pData;
}
...
{
    unsigned int d0,d1,d2; //dummy registers to show to gcc the old values are gone
#ifdef __XAP5_FAR__
#define BLKCPR  "blkcp.fr"
#else
#define BLKCPR  "blkcp.r "
#endif
    asm volatile (BLKCPR "   @(0, %0), @(0, %1), %2"
                  : "=r"(d0), "=r"(d1), "=r"(d2)
...
{
    unsigned int d0,d1; //dummy registers to show to gcc the old values are gone
#ifdef __XAP5_FAR__
#define BLKCPI  "blkcp.fi"
#else
#define BLKCPI  "blkcp.i "
#endif
    asm volatile (BLKCPI "   @(0, %0), @(0, %1), %4"
                  : "=r"(d0), "=r"(d1)
                  : "0"(dst), "1"(src) , "I"(Length)
                  : "memory"
                  );
#undef BLKCPI
}
...
ALWAYS_INLINE void VOLATILE_MEMCPY(void * dst, const void * src, UInt16 Length)
{
    ...
    {
        //allowed range for for #bytes in blkcp.i is [1..32]
        if ((Length >=1) && (Length <=32))
        {
            VOLATILE_MEMCPY_FIX(dst, src, Length); 
            return;
        }
    }
    VOLATILE_MEMCPY_VAR(dst, src, Length);
}

ALWAYS_INLINE void BLOCKREAD(UIntPtr Address, void * pBuffer, UInt16 Length)
{
    VOLATILE_MEMCPY(pBuffer, (void *) Address, Length);
}

ALWAYS_INLINE void BLOCKWRITE(UIntPtr Address, const void * pBuffer, UInt16 Length)
{
    VOLATILE_MEMCPY((void *) Address, pBuffer, Length);
}
